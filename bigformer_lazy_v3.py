"""BigFormer v7: Lazy loading — block 0 upfront, rest on demand.

Key: load only block 0 (804MB) upfront. Remaining 23 blocks loaded
from .data file on demand. Pipeline for overlap (optional).
"""
...
# ── Parse graph (0.001s, no weight data) ──
...
def load_block_gpu(names_dict):
    """Load one block from .data file to GPU (synchronous)."""
    result = {}
    for k, name in names_dict.items():
        if name is None: result[k] = None; continue
        t = load_one(name)
        result[k] = t.to(device) if t is not None else None
    return result

print(f"  Loading block 0...", file=sys.stderr)
blocks_gpu = [load_block_gpu(block_names[0])]
...
for start in range(0, N, BATCH_SIZE):
    ...
    for bid in range(num_blocks):
        # Load block on demand (lazy: only first time)
        if bid >= len(blocks_gpu):
            blocks_gpu.append(load_block_gpu(block_names[bid]))
        w = blocks_gpu[bid]
        ...
        # Free block weights after use (keep GPU memory low)
        if bid > 0:
            del blocks_gpu[bid-1]  # free previous block
...
print(f"\nTime: {dt:.1f}s | Precision: {'PASS' if ok else 'FAIL'} | MAX_DIFF: {np.max(np.abs(logits-golden)):.2e}", file=sys.stderr)
