...
class GPUExecutor:
    def __init__(self, onnx_path: str):
        ...
        self._shape_cache = {}  # tensor_name → shape list (cached)
        for node in model.graph.node:
            attrs = {}
            for a in node.attribute:
            ...
            try:
                if len(inp) >= 2 and inp[1] in self._reg:
                    ...
                    if shape_name in self._shape_cache:
                        vals = self._shape_cache[shape_name]
                    else:
                        vals = [int(v) for v in self._reg[shape_name].flatten().tolist()]
                        self._shape_cache[shape_name] = vals
