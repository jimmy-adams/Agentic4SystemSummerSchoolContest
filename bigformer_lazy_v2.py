"""BigFormer v7: Lazy loading — only block 0 upfront, rest on demand."""
...
# Load shared + block 0 only
def load_block(names_dict):
    return {k: load_one(v).to(device) if v else None for k, v in names_dict.items()}

tok_emb = load_one("tok_emb.weight").float().to(device) if rp("tok_emb.weight") else None
pos_emb = load_one("pos_emb").float().to(device) if rp("pos_emb") else None
...
def load_block_async(names_dict):
    ...
    for k, v in names_dict.items():
        ...
        with torch.cuda.stream(t_stream):
            result[k] = t.to(device, non_blocking=True) if t is not None else None
    return result
...
for start in range(0, N, BATCH_SIZE):
    ...
    for bid in range(num_blocks):
        ...
        # ── Pipeline: sync next, kick off next+1 ──
        if bid < num_blocks - 1:
