"""BigFormer Swap: blocks outer loop, sub-batches inner. 
FP32 weights converted ONCE per block, reused for all sub-batches.
"""
...
print("=== Loop Swap: blocks outer, sub-batches inner ===")
...
print(f"=== Inference (loop swap) ===")
torch.cuda.reset_peak_memory_stats()

# Phase 1: Embed all sub-batches first (batch=16)
sub_batches = []
for start in range(0, N, BATCH_SIZE):
    end = min(start + BATCH_SIZE, N)
    batch = input_ids[start:end].to(device)
    sub_batches.append(tok_emb[batch] + pos_emb[:, :batch.shape[1], :])

# Phase 2: Process blocks outer loop, sub-batches inner
for bid, w in enumerate(blocks_fp16):
    # Convert to FP32 ONCE for all sub-batches
    w_qkv = w["qkv"].float(); w_proj = w["proj"].float()
    w_ff1 = w["ff1"].float(); w_ff2 = w["ff2"].float()
    l1w = w["ln1_w"].float() if w["ln1_w"] is not None else None
    l1b = w["ln1_b"].float() if w["ln1_b"] is not None else None
    l2w = w["ln2_w"].float() if w["ln2_w"] is not None else None
    l2b = w["ln2_b"].float() if w["ln2_b"] is not None else None
    qb = w["qkv_b"].float() if w["qkv_b"] is not None else None
    pb = w["proj_b"].float() if w["proj_b"] is not None else None
    f1b = w["ff1_b"].float() if w["ff1_b"] is not None else None
    f2b = w["ff2_b"].float() if w["ff2_b"] is not None else None
    
    for si, x in enumerate(sub_batches):
        B, S = x.shape[0], x.shape[1]
        
        # LN1
        residual = x
        x = F.layer_norm(x, [D], weight=l1w, bias=l1b, eps=1e-5) if l1w is not None else x
        
        # QKV
        qkv = x @ w_qkv
        if qb is not None: qkv = qkv + qb
        ...
        if pb is not None: attn_out = attn_out + pb
        x = residual + attn_out
        
        # LN2 + FFN
        residual = x
        x = F.layer_norm(x, [D], weight=l2w, bias=l2b, eps=1e-5) if l2w is not None else x
        
        torch.backends.cuda.matmul.allow_tf32 = True
        x = x @ w_ff1
        if f1b is not None: x = x + f1b
        x = F.gelu(x)
        x = x @ w_ff2
        if f2b is not None: x = x + f2b
        torch.backends.cuda.matmul.allow_tf32 = False
        x = residual + x
        
        sub_batches[si] = x  # update in-place
    
    del w_qkv, w_proj, w_ff1, w_ff2
    if bid == 0: print(f"  Block 0 done: {time.perf_counter()-t0:.1f}s")
...
for si, x in enumerate(sub_batches):
    ...
    sub_batches[si] = x.cpu().numpy()

peak_gb = torch.cuda.max_memory_allocated() / 1e9
logits = np.concatenate(sub_batches, axis=0).astype(np.float32)
