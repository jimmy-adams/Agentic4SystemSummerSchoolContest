"""BigFormer AMP: FP16 weights + autocast compute, FP32 critical ops."""
import onnx
...
CACHE_PATH = "/tmp/bigformer_weights_fp16.pt"
BATCH_SIZE = 16
device = torch.device("cuda")

print("=== AMP: FP16 weights + autocast ===")
t0 = time.perf_counter()

# ── Load/cache weights in FP16 ──
if os.path.exists(CACHE_PATH):
    init_data = torch.load(CACHE_PATH, weights_only=True)
    for k in list(init_data.keys()):
        init_data[k] = init_data[k].pin_memory()
    print(f"  Cache load: {time.perf_counter()-t0:.1f}s")
else:
    ...
    for init in model.graph.initializer:
        ...
        if arr.dtype == np.float32:
            t = t.half()  # store as FP16
        init_data[init.name] = t.pin_memory()
    torch.save(init_data, CACHE_PATH)
    print(f"  ONNX load + cache save: {time.perf_counter()-t0:.1f}s")

total_gb = sum(t.numel() * t.element_size() for t in init_data.values()) / 1e9
print(f"  Weights: {total_gb:.1f} GB (FP16)")

# ── Parse graph (lightweight, no data) ──
model = onnx.load(ONNX_PATH)
identity_map = {}
...
# Pre-build blocks (FP16 weights, biases in FP32)
block_w_cpu = []
...
# Shared → GPU (FP32 for embeddings, FP16 for head)
...
for start in range(0, N, BATCH_SIZE):
    ...
    for bid in range(num_blocks):
        ...
        # ── QKV attention (FP32 — precision-critical) ──
        qkv = x @ w["qkv"].float()
        ...
        # Attention in FP32 (precision-critical)
